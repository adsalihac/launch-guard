...
function getCategory(title: string): string {
  ...
  if (t.includes("app store") || t.includes("apple") || t.includes("ios") || t.includes("iphone")) return "App Store";
  if (t.includes("google play") || t.includes("android")) return "Google Play";
  if (t.includes("privacy") || t.includes("data")) return "Privacy";
  if (t.includes("subscription") || t.includes("pay") || t.includes("pric")) return "Monetization";
  if (t.includes("reject") || t.includes("review") || t.includes("guideline") || t.includes("submission")) return "Review";
  return "Policy";
}
...
export default function Blog() {
  ...
  const filtered = useMemo(() => {
    let result = articles;
    if (activeCategory) {
      result = result.filter((a) => getCategory(a.title) === activeCategory);
    }
    if (search.trim()) {
    ...
    <main className="min-h-screen bg-[#0a0a0a] text-white">
      <nav className="fixed top-0 z-50 w-full border-b border-white/5 bg-[#0a0a0a]/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          ...
          <a href="/" className="text-sm text-gray-400 transition hover:text-white">Back to App</a>
        </div>
      </nav>
      ...
      <section className="relative overflow-hidden pt-32 pb-12 md:pt-40 md:pb-16">
        ...
        <div className="relative mx-auto max-w-4xl px-6 text-center">
          ...
          <h1 className="text-4xl font-extrabold leading-tight tracking-tight sm:text-5xl md:text-6xl">
            App Store & Google Play <br /><span className="text-brand-400">Industry News</span>
          </h1>
          <p className="mx-auto mt-4 max-w-2xl text-lg leading-relaxed text-gray-400">
            Curated from top industry sources. Stay updated on the latest app news,
            submission trends, and platform changes affecting your apps.
          </p>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-5xl px-6">
          {loading ? (
            <div className="space-y-6">
              <div className="h-14 animate-pulse rounded-xl border border-white/[0.06] bg-white/[0.02]" />
              <div className="h-56 animate-pulse rounded-2xl border border-white/[0.06] bg-white/[0.02]" />
              <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                {Array.from({ length: 6 }).map((_, i) => (
                  <div key={i} className="h-40 animate-pulse rounded-xl border border-white/[0.06] bg-white/[0.02]" />
                ))}
              </div>
            </div>
          ) : (
            <>
              <div className="mb-6 space-y-4">
                <div className="relative">
                  <svg className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <circle cx="11" cy="11" r="8" /><path d="m21 21-4.3-4.3" />
                  </svg>
                  <input
                    className="w-full rounded-xl border border-white/10 bg-white/[0.04] py-3 pl-11 pr-4 text-sm text-white outline-none transition placeholder:text-gray-600 focus:border-brand-400/30 focus:ring-4 focus:ring-brand-400/10"
              ...
              {filtered.length === 0 ? (
                <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-16 text-center">
                  ...
                  <p className="text-lg font-semibold text-gray-300">No matching articles</p>
                ...
                <>
                  {featured ? (
                    <a
                      ...
                      className="group relative mb-8 block overflow-hidden rounded-2xl border border-white/[0.06] bg-white/[0.02] p-6 transition hover:border-white/[0.12] sm:p-8"
                    >
                      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <div className="min-w-0 max-w-2xl">
                          <div className="mb-3 flex items-center gap-2">
                            <span className={`rounded-full border px-2.5 py-0.5 text-xs font-semibold ${categoryColors[getCategory(featured.title)]}`}>
                              {getCategory(featured.title)}
                            </span>
                            <span className="text-xs text-gray-600">Featured</span>
                          </div>
                          <h2 className="text-xl font-bold text-white transition group-hover:text-brand-400 sm:text-2xl line-clamp-2">{featured.title}</h2>
                          <div className="mt-4 flex items-center gap-3 text-sm text-gray-600">
                            <span className="rounded-md border border-white/5 bg-white/[0.03] px-2 py-0.5 text-xs">{featured.source}</span>
                            <span>{formatDate(featured.date)}</span>
                            <span className="flex items-center gap-1 text-brand-400 opacity-0 transition group-hover:opacity-100">
                              Read more <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 12h14M12 5l7 7-7 7" strokeLinecap="round" strokeLinejoin="round" /></svg>
                            </span>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    {rest.map((article, i) => (
                      ...
                      >
                        <span className={`mb-3 inline-block rounded-full border px-2.5 py-0.5 text-xs font-semibold ${categoryColors[getCategory(article.title)]}`}>
                          {getCategory(article.title)}
                        </span>
                        <h3 className="text-sm font-bold text-white transition group-hover:text-brand-400 line-clamp-2">{article.title}</h3>
      ...
      <section className="border-t border-white/5 py-20">
        <div className="mx-auto max-w-2xl px-6 text-center">
          <h2 className="text-2xl font-bold tracking-tight sm:text-3xl">Never miss a policy update</h2>
          ...
          <a href="/" className="mt-6 inline-flex items-center gap-2 rounded-xl bg-brand-400 px-6 py-3 text-sm font-bold text-black shadow-lg shadow-brand-400/20 transition hover:bg-brand-300">
